-- 2026_09_07_001_repos_schema.sql
-- ============================================================================
-- Repository (kr_*) + developer-billing baseline schema.
--
-- AUTO-GENERATED MIRROR of repos.db.ensureReposSchema()
-- (backend/src/modules/repos/repos.db.ts), which self-bootstraps the exact
-- same DDL at runtime. Regenerate with:
--   node backend/scratch/gen-repos-schema-sql.mjs
--
-- Why this file exists: migration 0004_repo_realtime creates triggers ON
-- kr_ci_runs / kr_deployments / kr_activity / kr_repositories. On a FRESH
-- database those tables only existed after runtime bootstrap, so 0004 failed
-- with: relation "kr_ci_runs" does not exist. This migration provisions the
-- full kr_* schema BEFORE 0004 runs, in the same order.
--
-- Every statement is idempotent (CREATE TABLE IF NOT EXISTS,
-- CREATE OR REPLACE FUNCTION, DROP TRIGGER IF EXISTS, ALTER TABLE ...
-- ADD COLUMN IF NOT EXISTS), so existing databases where the runtime
-- bootstrap already ran are a no-op and stay compatible.
-- ============================================================================
