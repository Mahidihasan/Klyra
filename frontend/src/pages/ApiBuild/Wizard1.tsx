...
export const StepNewProject: React.FC<{ init: CreateProjectInput; onNext: (v: CreateProjectInput) => void; onBack: () => void }> = ({ init, onNext, onBack }) => {
  ...
  return <WizardChrome step={0} total={8} labels={STEPS}>
    ...
    <div className="ab2-project-form-grid">
      ...
      <aside className="ab2-project-preview"><span className="ab2-preview-label">Marketplace preview</span><div className="ab2-preview-mark" aria-hidden="true">{value.name ? value.name.charAt(0).toUpperCase() : <Globe2 size={21} />}</div><h3>{value.name || 'Your API name'}</h3><p>{value.description || 'A clear description will help consumers understand what this API does.'}</p><div><span className="ab2-pill">{value.category || 'Choose a category'}</span><span className="ab2-pill ab2-mono">v1.0.0</span></div><div className="ab2-preview-url"><span>Gateway URL</span><code>localhost:4000/api/gateway/{slug}</code></div></aside>
    </div>
