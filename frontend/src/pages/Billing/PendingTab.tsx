...
const COPY = {
  ...
  info: {
    ...
    body: 'Billing name, address, invoice email and tax ID have nowhere to live yet — the schema has no fields for them.',
    blockers: [
      'No billing address or tax ID columns in the schema',
      'Open question: store on users.metadata (JSONB) or a new table?',
      'Invoice email may differ from the account email',
    ],
  },
} as const;
...
export const PendingTab: React.FC<PendingTabProps> = ({ kind }) => {
  ...
  const Icon = kind === 'methods' ? CreditCard : MapPin;
