// Wallet shapes, mirrored from backend/src/modules/wallet/wallet.types.ts.
//
// Everything below this header is identical in both files on purpose. They are
// the API contract, and the two workspaces compile separately — so a field
// renamed on one side and not the other is a bug no type-checker will catch.
